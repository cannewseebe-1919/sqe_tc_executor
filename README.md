...
```
[단말 내 Runner App] → WebSocket 연결 (ws://서버IP:9000/ws/runner)
                           ↓
                    [백엔드 /ws/runner 엔드포인트]
...
```
[브라우저]
    → WebSocket 연결: ws://서버:9000/api/execute/{id}/stream
...
cd backend
source venv/bin/activate
PYTHONPATH=. uvicorn app.main:app --host 0.0.0.0 --port 9000
```
...
{
  "sp": {
    "entityId": "http://이-서버-IP:9000/api/auth/saml/metadata",
    "assertionConsumerService": {
      "url": "http://이-서버-IP:9000/api/auth/saml/acs",
      "binding": "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
    },
    "singleLogoutService": {
      "url": "http://이-서버-IP:9000/api/auth/saml/slo",
...
2. **Server URL** 입력란에 executor 백엔드 WebSocket 주소 입력:
   ```
   ws://localhost:9000/ws/runner
   ```
   > USB 연결 시 백엔드가 `adb reverse tcp:9000 tcp:9000`을 자동으로 설정하므로 `localhost`를 사용합니다.
3. **Connect** 버튼 탭
...
| 서비스 | 포트 | 실행 방식 | 설명 |
|--------|------|-----------|------|
| 백엔드 API | **9000** | 호스트 직접 실행 | sqe_tc_bot에서 호출 |
...
이후: Runner App --[WebSocket ws://localhost:9000/ws/runner]--> 백엔드
```

**adb reverse 자동 설정**

사내 Wi-Fi 환경에서 단말이 서버 IP로 직접 접근할 수 없는 경우를 위해,  
단말 연결(또는 재연결) 시 백엔드가 자동으로 `adb reverse tcp:9000 tcp:9000`을 실행합니다.  
이를 통해 Runner App은 `ws://localhost:9000/ws/runner`로 연결하면 됩니다.
...
| | 이전 (v1.1) | v1.2 |
|-|------------|------|
| Runner App 서버 URL | `ws://서버IP:9000/ws/runner` | `ws://localhost:9000/ws/runner` |
